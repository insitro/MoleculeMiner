...
def compute_tanimoto_similarities(gold_smiles, pred_smiles, num_workers=16):
    with multiprocessing.Pool(num_workers) as p:
        similarities = p.starmap(
            tanimoto_similarity, [(gs, ps) for gs, ps in zip(gold_smiles, pred_smiles)]
        )
    return similarities
...
class SmilesEvaluator(object):
    ...
    def _replace_empty(self, smiles_list):
        """
        Replace empty SMILES in the gold, otherwise it will be considered correct if both
        pred and gold is empty.
        """
        return [
            smiles
    ...
    def evaluate(self, pred_smiles, include_details=False):
        results = {}
        if self.tanimoto:
            results["tanimoto"] = np.mean(
                compute_tanimoto_similarities(self.gold_smiles, pred_smiles)
            )
        ...
        results["canon_smiles"] = np.mean(
            np.array(self.gold_smiles_cistrans) == np.array(pred_smiles_cistrans)
        )
        if include_details:
            results["canon_smiles_details"] = np.array(self.gold_smiles_cistrans) == np.array(
                pred_smiles_cistrans
            )
    ...
    for image_id in gold_df["image_id"]:
        ...
        if image_id not in image2predidx:
            pred_df = pred_df.append(
                {"image_id": image_id, args.pred_field: ""}, ignore_index=True
            )
    image2predidx = {image_id: idx for idx, image_id in enumerate(pred_df["image_id"])}
