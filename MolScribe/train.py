...
from molscribe.chemistry import convert_graph_to_smiles, keep_main_molecule, postprocess_smiles
from molscribe.dataset import AuxTrainDataset, TrainDataset, bms_collate
...
def get_args():
    ...
    parser.add_argument("--mask_pos_emb", type=int, default=1)
    ...
    group.add_argument(
        "--dec_num_layers", help="No. of layers in transformer decoder", type=int, default=6
    )
    group.add_argument("--dec_hidden_size", help="Decoder hidden size", type=int, default=256)
    group.add_argument(
        "--dec_attn_heads", help="Decoder no. of attention heads", type=int, default=8
    )
    ...
    group.add_argument(
        "--max_relative_positions", help="Max relative positions", type=int, default=0
    )
    # Data
    parser.add_argument("--data_path", type=str, default=None)
    parser.add_argument(
        "--data_path_decoder", type=str, default="selected_weights/swin_base_char_aux_1m680k.pth"
    )
    parser.add_argument("--train_file", type=str, default=None)
    parser.add_argument("--train_v1", type=int, default=1)
    parser.add_argument("--perturb", action="store_true")
    parser.add_argument("--perturb_ratio", type=float, default=0.02)
    parser.add_argument("--do_mops", action="store_true")
    ...
    parser.add_argument(
        "--all_data", action="store_true", help="Use both train and valid data for training."
    )
    ...
    parser.add_argument("--custom_enc", action="store_true")
    # Inference
...
def load_custom(load_path):
    print_rank_0("LOADING only Pre-trained Decoder weights for custom SWIN...")
...
def safe_load(module, module_states):
    ...
    missing_keys, unexpected_keys = module.load_state_dict(
        remove_prefix(module_states), strict=False
    )
    if missing_keys:
...
def get_model(args, tokenizer, device, load_path=None):
    ...
    elif args.custom_enc:  # Load only decoder weights for now
        states = load_custom(args.data_path_decoder)
        safe_load(decoder, states["decoder"])
        print_rank_0("Model loaded from swin_base_char_aux_1m680k.pth")
...
def get_optimizer_and_scheduler(args, encoder, decoder, load_path=None):
    encoder_optimizer = AdamW(
        encoder.parameters(), lr=args.encoder_lr, weight_decay=args.weight_decay, amsgrad=False
    )
    encoder_scheduler = get_scheduler(
        args.scheduler, encoder_optimizer, args.num_warmup_steps, args.num_training_steps
    )

    decoder_optimizer = AdamW(
        decoder.parameters(), lr=args.decoder_lr, weight_decay=args.weight_decay, amsgrad=False
    )
    decoder_scheduler = get_scheduler(
        args.scheduler, decoder_optimizer, args.num_warmup_steps, args.num_training_steps
    )
...
def train_fn(
    ...
    for step, (indices, images, refs, pixel_maps) in enumerate(train_loader):
        # measure data loading time
        data_time.update(time.time() - end)
        images = images.to(device)
        pixel_maps = [pmap.to(device) for pmap in pixel_maps]
        batch_size = images.size(0)
        with torch.cuda.amp.autocast(enabled=args.fp16):
            features, hiddens = encoder(images, refs, pixel_maps)
        ...
        # Clear results and loss to conserve GPU memory
        del results
        del loss
        ...
        loss_dict = {k: v.avg for k, v in loss_meter.subs.items()}
        write_to_tb(
            epoch,
            step,
            total_steps,
        ...
        if step % args.print_freq == 0 or step == (len(train_loader) - 1):
            ...
            print_rank_0(
                ...
                "LR: {encoder_lr:.6f} {decoder_lr:.6f}".format(
                    epoch + 1,
                    step,
                    len(train_loader),
                    data_time=data_time,
            ...
            loss_meter.reset()
        if (
            args.train_steps_per_epoch != -1
            and (step + 1) // args.gradient_accumulation_steps == args.train_steps_per_epoch
        ):
            break
...
def valid_fn(valid_loader, encoder, decoder, tokenizer, device, args):
    ...
    predictions = {}
    start = end = time.time()
    # Inference is distributed. The batch is divided and run independently on multiple GPUs,
    # and the predictions are gathered afterwards.
    for step, (indices, images, refs, pixel_maps) in enumerate(valid_loader):
        # measure data loading time
        data_time.update(time.time() - end)
        images = images.to(device)
        pixel_maps = [pmap.to(device) for pmap in pixel_maps]
...
def train_loop(args, train_df, valid_df, aux_df, tokenizer, save_path):
    ...
    if aux_df is None:
        train_dataset = TrainDataset(
            args, train_df, tokenizer, split="train", dynamic_indigo=args.dynamic_indigo
        )
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # eval
        scores = validation(args, valid_df, tokenizer, encoder, decoder, save_path, split="valid")

        if args.local_rank != 0 or args.global_rank != 0:
            continue
        ...
        if score >= best_score:
            ...
            torch.save(
                save_obj, os.path.join(save_path, f"{args.encoder}_{args.decoder}_best.pth")
            )
        ...
        if args.save_mode == "all":
            torch.save(
                save_obj, os.path.join(save_path, f"{args.encoder}_{args.decoder}_ep{epoch}.pth")
            )
        if args.save_mode == "last":
            torch.save(
                save_obj, os.path.join(save_path, f"{args.encoder}_{args.decoder}_last.pth")
            )
...
def validation(args, data_df, tokenizer, encoder=None, decoder=None, save_path=None, split="test"):  # noqa: C901
    ...
    if split == "test":
        with open(
            os.path.join(
                save_path, f"eval_scores_{os.path.splitext(file)[0]}_{args.load_ckpt}.json"
            ),
            "w",
        ) as f:
            json.dump(scores, f)
...
def get_chemdraw_data(args):
    ...
    if args.do_train:
        ...
        if args.train_v1:
            train_df = pd.concat(
                [pd.read_csv(os.path.join(args.data_path, file)) for file in train_files]
            )
        else:  # V2: Generated 5.1M
            train_df = pd.read_csv(os.path.join(args.data_path, args.train_file), header=None)
            train_df.columns = ["SMILES"]
...
def write_to_tb(epoch, step, total_steps, loss_dict, enc_norm, dec_norm, enc_lr, dec_lr, writer):
    if torch.distributed.is_initialized():
        if torch.distributed.get_rank() == 0:
            write_scalars(
                epoch, step, total_steps, loss_dict, enc_norm, dec_norm, enc_lr, dec_lr, writer
            )
            writer.flush()
    else:
        write_scalars(
...
def write_scalars(epoch, step, total_steps, loss_dict, enc_norm, dec_norm, enc_lr, dec_lr, writer):
    global_step = (epoch * total_steps) + step
    writer.add_scalar("Total Loss", sum(loss_dict.values()), global_step)

    for k in loss_dict.keys():
        writer.add_scalar(k, loss_dict[k], global_step)

    writer.add_scalar("Encoder Norm", enc_norm, global_step)
    writer.add_scalar("Decoder Norm", dec_norm, global_step)
    writer.add_scalar("Encoder LR", enc_lr, global_step)
    writer.add_scalar("Decoder LR", dec_lr, global_step)
...
def main():
    ...
    if args.local_rank != -1:
        dist.init_process_group(
            backend=args.backend, init_method="env://", timeout=datetime.timedelta(0, 14400)
        )
    ...
    if args.do_valid:
        scores = validation(args, valid_df, tokenizer, save_path=args.save_path, split="test")
        print_rank_0(json.dumps(scores, indent=4))

    if args.do_test:
        assert isinstance(test_df, list)
        for df in test_df:
            scores = validation(args, df, tokenizer, save_path=args.save_path, split="test")
            print_rank_0(json.dumps(scores, indent=4))
