...
class NodeTokenizer(Tokenizer):
    def __init__(
        self, input_size=100, path=None, sep_xy=False, continuous_coords=False, debug=False
    ):
        super().__init__(path)
    ...
    def sequence_to_nodes(self, sequence):
        ...
        while i + 2 < len(sequence):
            if sequence[i] == EOS_ID:
                break
            if (
                self.is_x(sequence[i])
                and self.is_y(sequence[i + 1])
                and self.is_symbol(sequence[i + 2])
            ):
        ...
        for i, label in enumerate(sequence):
            ...
            if self.is_atom_token(token):
                if has_coords:
                    if (
                        i + 3 < len(sequence)
                        and self.is_x(sequence[i + 1])
                        and self.is_y(sequence[i + 2])
                    ):
...
def get_tokenizer(args):
    ...
    for format_ in args.formats:
        if format_ == "atomtok":
            ...
            if args.vocab_file is None:
                args.vocab_file = os.path.join(os.path.dirname(__file__), "vocab/vocab_uspto.json")
            tokenizer["atomtok_coords"] = NodeTokenizer(
                args.coord_bins,
                args.vocab_file,
                args.sep_xy,
            ...
            tokenizer["chartok_coords"] = CharTokenizer(
                args.coord_bins,
                args.vocab_file,
                args.sep_xy,
                continuous_coords=args.continuous_coords,
