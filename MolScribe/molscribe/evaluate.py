...
# AllChem.AtomKekulizeException
#         or AllChem.AtomSanitizeException
#         or AllChem.KekulizeException
#         or AllChem.MolSanitizeException
#         or Chem.AtomValenceException
...
class SmilesEvaluator(object):
    def __init__(self, gold_smiles, num_workers=16):
        ...
        self.gold_canon_smiles, self.gold_valid = convert_smiles_to_canonsmiles(
            gold_smiles, num_workers=num_workers
        )
        self.gold_smiles_chiral, _ = convert_smiles_to_canonsmiles(
            gold_smiles, ignore_chiral=True, num_workers=num_workers
        )
    ...
    def _replace_empty(self, smiles_list):
        """
        Replace empty SMILES in the gold, otherwise it will be considered correct
        if both pred and gold is empty.
        """
        return [
            smiles
            if smiles is not None and isinstance(smiles, str) and smiles != ""
            else "<empty>"
    ...
    def evaluate(self, pred_smiles):
        ...
        results["canon_smiles_em"] = (
            np.array(self.gold_canon_smiles) == np.array(pred_canon_smiles)
        ).mean()
        ...
        results["graph"] = (
            np.array(self.gold_smiles_chiral) == np.array(pred_smiles_chiral)
        ).mean()
        # Ignore double bond cis/trans
        pred_smiles_cistrans, _ = convert_smiles_to_canonsmiles(pred_smiles, ignore_cistrans=True)
        results["canon_smiles"] = (
            np.array(self.gold_smiles_cistrans) == np.array(pred_smiles_cistrans)
        ).mean()
        # Evaluate on molecules with chiral centers
        chiral = np.array(
            [[g, p] for g, p in zip(self.gold_smiles_cistrans, pred_smiles_cistrans) if "@" in g]
        )
