...
from dataset import GenerateMaps, get_transforms
...
class MolScribe:
    def __init__(self, model_path, device=None):
        ...
        self.encoder, self.decoder = self._get_model(
            args, self.tokenizer, self.device, model_states
        )
        self.transform = get_transforms(args.input_size, augment=False)
        self.downscaling_factors = (4, 2, 2, 2)
    ...
    def _get_args(self, args_states=None):
        ...
        parser.add_argument(
            "--trunc_encoder", action="store_true"
        )  # use the hidden states before downsample
        ...
        parser.add_argument("--custom_enc", type=int, default=1)
    ...
    def predict_images(
        self, input_images: List, return_atoms_bonds=False, return_confidence=False, batch_size=16
    ):
        device = self.device
        predictions = []
        ...
        for idx in range(0, len(input_images), batch_size):
            ...
            # Create and collate the new pixel maps
            pixel_maps = [self.create_maps(image) for image in batch_images]
            stages = [[] for i in range(4)]  # type: ignore
            for pmap in pixel_maps:
                for i, stage in enumerate(pmap):
                    stages[i].append(stage)
            maps_final = []
            for stage in stages:
                maps_final.append(torch.stack(stage).type(torch.FloatTensor))  # type: ignore

            # Send the images and pmaps to the GPU
            images_tensor = torch.stack(images, dim=0).to(device)
            pixel_maps = [pmap.to(device) for pmap in maps_final]
            with torch.no_grad():
                features, hiddens = self.encoder(images_tensor, pixel_maps=pixel_maps)
    ...
    def predict_image(self, image, return_atoms_bonds=False, return_confidence=False):
        return self.predict_images(
            [image], return_atoms_bonds=return_atoms_bonds, return_confidence=return_confidence
        )[0]

    def predict_image_files(
        self, image_files: List, return_atoms_bonds=False, return_confidence=False, batch_size=32
    ):
        input_images = []
    ...
    def predict_image_file(
        self, image_file: str, return_atoms_bonds=False, return_confidence=False
    ):
