...
from molscribe.constants import ABBREVIATIONS, FORMULA_REGEX, RGROUP_SYMBOLS, VALENCES
...
def _verify_chirality(mol, coords, symbols, edges, debug=False):
    try:
        ...
        # So we do this first, remove the conformer and add back the 2D conformer for
        # chiral correction
...
def list_to_smi(formula_list, start_bond, end_bond=None, direction=None):  # noqa: C901
    """
    Converts condensed formula (in the form of a list of symbols) to smiles
    Input:
    `formula_list`: e.g. ['C', 'H', 'H', 'N', ['C', 'H', 'H', 'H'], ['C', 'H', 'H', 'H']]
                    for CH2N(CH3)2
    `start_bond`: # bonds attached to beginning of formula
    `end_bond`: # bonds attached to end of formula (deduce automatically if None)
    `direction` (1, -1, or None): direction in which to process the list (1: left to right;
                -1: right to left; None: deduce automatically)
    Returns:
    `smiles`: smiles corresponding to input condensed formula
    `bonds_left`: bonds remaining at the end of the formula (for connecting back to main
                    molecule); should equal `end_bond` if specified
    ...
    if direction is None:
        ...
        for dir_choice in [1, -1]:
            smiles, bonds_left, trials, success = list_to_smi(
    ...
    def dfs(smiles, bonds_left, cur_idx, add_idx):
        ...
        `cur_idx`: index (in list `formula`) of current atom (i.e. atom to which subsequent
                    atoms are being attached)
        ...
        if (direction == 1 and add_idx == len(formula_list)) or (
            direction == -1 and add_idx == -1
        ):
        ...
        if isinstance(
            to_add, list
        ):  # "atom" added is a list (i.e. nested condensed formula): assume valence of 1
            if bonds_left > 1:
                # "atom" added does not use up remaining bonds of current atom
                # get smiles of "atom" (which is itself a condensed formula)
                add_str, val, trials, success = list_to_smi(to_add, 1, None, direction)
                if val > 0:
                    add_str = _get_bond_symb(val + 1) + add_str
                num_trials += trials
                if not success:
                    return smiles, bonds_left, num_trials, False
                # put smiles of "atom" in parentheses and append to smiles; go to next atom to
                # add to current atom
                ...
                add_str, bonds_left, trials, success = list_to_smi(to_add, 1, None, direction)
                num_trials += trials
                if not success:
                    return smiles, bonds_left, num_trials, False
                # append smiles of "atom" (without parentheses) to smiles; it becomes
                # new current atom
        ...
        for val in VALENCES.get(to_add, [1]):  # try all possible valences of atom added
            ...
            if (
                bonds_left > val
            ):  # atom added does not use up remaining bonds of current atom; go to next atom to
                # add to current atom
                if cur_idx >= 0:
                    add_str = _get_bond_symb(val) + add_str
                result = dfs(
                    smiles + f"({add_str})", bonds_left - val, cur_idx, add_idx + direction
                )
...
def get_smiles_from_symbol(symbol, mol, atom, bonds):
    ...
    smiles, bonds_left, num_trails, success = list_to_smi(formula_list, total_bonds, None)
    if success:
        return smiles
    return None
...
def _replace_functional_group(smiles):
    ...
    # where isotope is an identifier.
...
def _expand_functional_group(mol, mappings, debug=False):
    def _need_expand(mol, mappings):
        return (
            any([len(Chem.GetAtomAlias(atom)) > 0 for atom in mol.GetAtoms()]) or len(mappings) > 0
        )
    ...
    if _need_expand(mol, mappings):
        ...
        for i in range(num_atoms):
            ...
            if atom.GetSymbol() == "*":
                ...
                adjacent_atoms = [
                    mol_w.GetAtomWithIdx(adjacent_idx) for adjacent_idx in adjacent_indices
                ]
...
def graph_to_smi(coords, symbols, edges, image=None, debug=False):  # noqa: C901
    mol = Chem.RWMol()
    n = len(symbols)
    ids = []
    for i in range(n):
        ...
        if symbol in RGROUP_SYMBOLS:
            ...
            try:  # try to get SMILES of atom
                ...
                # otherwise, abbreviation or condensed formula
                atom = Chem.Atom("*")
                Chem.SetAtomAlias(atom, symbol)
    ...
    try:
        ...
        # molblock is obtained before expanding func groups, otherwise the
        # expanded group won't have coordinates.
...
def convert_graph_to_smiles(coords, symbols, edges, images=None, num_workers=16):
    ...
    if num_workers <= 1:
        results = itertools.starmap(graph_to_smi, args_zip)
        results = list(results)
    else:
        with multiprocessing.Pool(num_workers) as p:
            results = p.starmap(graph_to_smi, args_zip, chunksize=128)
...
def _postprocess_smiles(
    smiles, coords=None, symbols=None, edges=None, molblock=False, debug=False
):
...
def postprocess_smiles(
    smiles, coords=None, symbols=None, edges=None, molblock=False, num_workers=16
):
    with multiprocessing.Pool(num_workers) as p:
        if coords is not None and symbols is not None and edges is not None:
            results = p.starmap(
                _postprocess_smiles, zip(smiles, coords, symbols, edges), chunksize=128
            )
        else:
