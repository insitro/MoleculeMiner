...
class GreedySearch(DecodeStrategy):
    ...
    def initialize(self, memory_bank, device=None):
        ...
        self.select_indices = torch.arange(self.batch_size, dtype=torch.long, device=device)
        self.original_batch_idx = torch.arange(self.batch_size, dtype=torch.long, device=device)
    ...
    def advance(self, log_probs, attn=None, hidden=None, label=None):
        ...
        topk_ids, self.topk_scores = self._pick(
            log_probs
        )  # log_probs: b x v; topk_ids & self.topk_scores: b x (t=1)
        self.is_finished = topk_ids.eq(self.eos)
        if label is not None:
            label = label.view_as(self.is_finished)
            self.is_finished = label.eq(self.eos)
        self.alive_seq = torch.cat(
            [self.alive_seq, topk_ids], -1
        )  # b x (l+1) (first element is <bos>; note l = len(self)-1)
        self.alive_log_token_scores = torch.cat(
            [self.alive_log_token_scores, self.topk_scores], -1
        )
    ...
    def update_finished(self):
        ...
        # is_finished indicates the decoder finished generating the sequence.
        # Remove it from the batch and update the results.
        ...
        for b in finished_batches.view(-1):
            ...
            self.scores[b_orig].append(
                torch.exp(torch.mean(self.alive_log_token_scores[b])).item()
            )
