...
import cv2
import numpy as np
from albumentations.augmentations.geometric.functional import keypoint_rotate
...
def safe_rotate(
    ...
    rot_trans = A.Affine(rotate=[angle, angle], p=1, mode=cv2.BORDER_CONSTANT, fit_output=True)
    # safe_rotate_enlarged_img_size(angle=angle, rows=old_rows, cols=old_cols)
    img = rot_trans(image=img)["image"]
...
def keypoint_safe_rotate(keypoint, angle, rows, cols):
    ...
    # Rows and columns of the rotated image (not cropped)
    rot_trans = A.Affine(rotate=[angle, angle], p=1, mode=cv2.BORDER_CONSTANT, fit_output=True)
    # Create a dummy image for rotation
    dum_img = np.zeros((old_rows, old_cols, 3), dtype=np.uint8)
    dum_rot = rot_trans(image=dum_img)["image"]
    new_rows, new_cols = dum_rot.shape[:2]
    ...
    # Shift keypoint
    shifted_keypoint = (
        int(keypoint[0] + col_diff),
        int(keypoint[1] + row_diff),
        keypoint[2],
        keypoint[3],
    )
...
class CropWhite(A.DualTransform):
    ...
    def apply_to_keypoint(
        self, keypoint, crop_top=0, crop_bottom=0, crop_left=0, crop_right=0, **params
    ):
...
class ResizePad(A.DualTransform):
    ...
    def apply(self, img, interpolation=cv2.INTER_LINEAR, **params):
        ...
        img = A.augmentations.geometric.functional.resize(
            img, height=min(h, self.height), width=min(w, self.width), interpolation=interpolation
        )
        h, w, _ = img.shape
...
def normalized_grid_distortion(img, num_steps=10, xsteps=(), ysteps=(), *args, **kwargs):
    ...
    return A.augmentations.functional.grid_distortion(
        img, num_steps, xsteps, ysteps, *args, **kwargs
    )


# class NormalizedGridDistortion(A.augmentations.transforms.GridDistortion):
#     def apply(self, img, stepsx=(), stepsy=(), interpolation=cv2.INTER_LINEAR, **params):
#         return normalized_grid_distortion(img, self.num_steps, stepsx,
#                   stepsy, interpolation, self.border_mode,
#                                           self.value)

#     def apply_to_mask(self, img, stepsx=(), stepsy=(), **params):
#         return normalized_grid_distortion(
#             img, self.num_steps, stepsx, stepsy, cv2.INTER_NEAREST,
