...
from molscribe.inference import BeamSearch, GreedySearch
from molscribe.swin_custom import swin_b
from molscribe.tokenizer import EOS_ID, MASK_ID, PAD_ID, SOS_ID
from molscribe.transformer import Embeddings, TransformerDecoder
from molscribe.utils import FORMAT_INFO, to_device
...
class Encoder(nn.Module):
    def __init__(self, args, pretrained=False):
        ...
        if model_name.startswith("resnet"):
            ...
            # MolScribe V2
                ...
                self.transformer = swin_b(window_size=12, mask_pos_emb=args.mask_pos_emb)
    ...
    def forward(self, x, refs=None, pixel_maps=None):
        if self.model_type in ["resnet", "efficientnet"]:
            ...
            if "patch" in self.model_name:
                ...
                features, hiddens = self.transformer(x, pixel_maps)
        else:
            raise NotImplementedError
...
class TransformerDecoderAR(TransformerDecoderBase):
    ...
    def forward(self, encoder_out, labels, label_lengths):
        ...
        dec_out, *_ = self.decoder(
            tgt_emb=tgt_emb, memory_bank=memory_bank, tgt_pad_mask=tgt_pad_mask
        )
    ...
    def decode(
        ...
        Only greedy search is supported now. Beam search is
        out-dated. The labels is used for partial prediction,
        i.e. part of the sequence is given. In standard decoding,
        labels=None.
        """
        batch_size, max_len, _ = encoder_out.size()
        memory_bank = self.enc_transform(encoder_out)
        ...
        for step in range(decode_strategy.max_length):
            ...
            dec_out, dec_attn, *_ = self.decoder(
                tgt_emb=tgt_emb, memory_bank=memory_bank, tgt_pad_mask=tgt_pad_mask, step=step
            )
            ...
            if self.tokenizer.output_constraint:
                ...
                output_mask_tensor = torch.tensor(output_mask, device=log_probs.device)
                log_probs.masked_fill_(output_mask_tensor, -10000)

            label = (
                labels[:, step + 1] if labels is not None and step + 1 < labels.size(1) else None
            )
        ...
        if orig_labels is not None:
            for i in range(batch_size):
                pred = results["predictions"][i][0]  # type: ignore
                label = orig_labels[i][1 : len(pred) + 1]
                mask = label.eq(MASK_ID).long()
                pred = pred[: len(label)]
                results["predictions"][i][0] = pred * mask + label * (1 - mask)  # type: ignore
    ...
    def __init__(self, decoder_dim, coords=False):
        ...
        if coords:
            self.coords_mlp = nn.Sequential(
                nn.Linear(decoder_dim, decoder_dim), nn.GELU(), nn.Linear(decoder_dim, 2)
            )

    def forward(self, hidden, indices=None):
        b, latent, dim = hidden.size()
        if indices is None:
            index = [i for i in range(3, latent, 3)]
            hidden = hidden[:, index]
        else:
        ...
        hh = torch.cat(
            [
                hidden.unsqueeze(2).expand(b, latent, latent, dim),
                hidden.unsqueeze(1).expand(b, latent, latent, dim),
            ],
            dim=3,
        )
...
class Decoder(nn.Module):
        ...
        for format_ in args.formats:
            if format_ == "edges":
                decoder["edges"] = GraphPredictor(
                    args.dec_hidden_size, coords=args.continuous_coords
                )
            else:
    ...
    def decode(self, encoder_out, hiddens=None, refs=None, beam_size=1, n_best=1):
        """
        Inference mode. Call each decoder's decode method (if required), convert the
        output format (e.g. token to sequence). Beam search is not supported yet.
        """
        results = {}
        predictions = []
        for format_ in self.formats:
            if format_ in ["atomtok", "atomtok_coords", "chartok_coords"]:
                ...
                results[format_] = self.decoder[format_].decode(
                    encoder_out, beam_size, n_best, max_length=max_len
                )
                outputs, scores, token_scores, *_ = results[format_]
                beam_preds = [
                    [self.tokenizer[format_].sequence_to_smiles(x.tolist()) for x in pred]
                    for pred in outputs
                ]
                    ...
                    for i in range(len(predictions)):
                        ...
                        if format_ == "chartok_coords":
                            ...
                            for symbol, index in zip(predictions[i][format_]["symbols"], indices):
                                atom_score = (
                                    np.prod(
                                        token_scores[i][0][index - len(symbol) + 1 : index + 1]
                                    )
                                    ** (1 / len(symbol))
                                ).item()
            ...
            if format_ == "edges":
                ...
                for i in range(len(dec_out)):
                    ...
                    indices = torch.LongTensor(predictions[i][atom_format]["indices"]).unsqueeze(
                        0
                    )  # 1 * k
                    pred = self.decoder["edges"](hidden, indices)  # k * k
                    prob = F.softmax(
                        pred["edges"].squeeze(0).permute(1, 2, 0), dim=2
                    ).tolist()  # k * k * 7
                    ...
                    if self.compute_confidence:
                        ...
                        predictions[i]["overall_score"] = (
                            predictions[i][atom_format]["average_token_score"]
