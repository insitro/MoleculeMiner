...
from . import IndigoException  # noqa
...
class IndigoRenderer(object):
    def __init__(self, indigo):
        ...
        if (
            ...
            self._lib = CDLL(indigo.dllpath + r"\indigo-renderer.dll")
    ...
    def renderGridToBuffer(self, objects, refatoms, ncolumns):
        ...
        wb = self.indigo.writeBuffer()
        try:
            self.indigo._checkResult(self._lib.indigoRenderGrid(objects.id, arr, ncolumns, wb.id))
            return wb.toBuffer()
        finally:
            wb.dispose()
