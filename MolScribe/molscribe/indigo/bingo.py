...
class Bingo(object):
    def __init__(self, bingoId, indigo, lib):
        ...
        self._lib.bingoSearchSimWithExtFP.argtypes = [
            c_int,
            c_int,
            c_float,
            c_float,
        ...
        self._lib.bingoSearchSimTopNWithExtFP.argtypes = [
            c_int,
            c_int,
            c_int,
            c_float,
    ...
    def createDatabaseFile(indigo, path, databaseType, options=""):
        ...
        return Bingo(
            Bingo._checkResult(
                indigo,
                lib.bingoCreateDatabaseFile(
                    path.encode("ascii"), databaseType.encode("ascii"), options.encode("ascii")
                ),
            ),
            indigo,
            lib,
    ...
    def loadDatabaseFile(indigo, path, options=""):
        ...
        return Bingo(
            Bingo._checkResult(
                indigo, lib.bingoLoadDatabaseFile(path.encode("ascii"), options.encode("ascii"))
            ),
            indigo,
            lib,
        )
    ...
    def insert(self, indigoObject, index=None):
        self._indigo._setSessionId()
        if not index:
            return Bingo._checkResult(
                self._indigo, self._lib.bingoInsertRecordObj(self._id, indigoObject.id)
            )
        else:
            return Bingo._checkResult(
                self._indigo,
    ...
    def insertWithExtFP(self, indigoObject, ext_fp, index=None):
        ...
        if not index:
            return Bingo._checkResult(
                self._indigo,
                self._lib.bingoInsertRecordObjWithExtFP(self._id, indigoObject.id, ext_fp.id),
            )
        else:
            return Bingo._checkResult(
                self._indigo,
    ...
    def searchSub(self, query, options=""):
        ...
        return BingoObject(
            Bingo._checkResult(
                self._indigo, self._lib.bingoSearchSub(self._id, query.id, options.encode("ascii"))
            ),
            self._indigo,
            self,
        )
    ...
    def searchExact(self, query, options=""):
        ...
        return BingoObject(
            Bingo._checkResult(
                self._indigo,
                self._lib.bingoSearchExact(self._id, query.id, options.encode("ascii")),
            ),
            self._indigo,
            self,
        )
        ...
        return BingoObject(
            Bingo._checkResult(
                ...
                self._lib.bingoSearchSim(
                    self._id, query.id, minSim, maxSim, metric.encode("ascii")
                ),
            ),
            self._indigo,
            self,
        ...
        return BingoObject(
            Bingo._checkResult(
                ...
                self._lib.bingoSearchSimWithExtFP(
                    self._id, query.id, minSim, maxSim, ext_fp.id, metric.encode("ascii")
                ),
            ),
            self._indigo,
            self,
        ...
        return BingoObject(
            Bingo._checkResult(
                ...
                self._lib.bingoSearchSimTopN(
                    self._id, query.id, limit, minSim, metric.encode("ascii")
                ),
            ),
            self._indigo,
            self,
        ...
        return BingoObject(
            Bingo._checkResult(
                ...
                self._lib.bingoSearchSimTopNWithExtFP(
                    self._id, query.id, limit, minSim, ext_fp.id, metric.encode("ascii")
                ),
            ),
            self._indigo,
            self,
        ...
        return BingoObject(
            Bingo._checkResult(
                ...
                self._lib.bingoSearchMolFormula(
                    self._id, query.encode("ascii"), options.encode("ascii")
                ),
            ),
            self._indigo,
            self,
    ...
    def getRecordById(self, id):
        ...
        return IndigoObject(
            self._indigo,
            Bingo._checkResult(self._indigo, self._lib.bingoGetRecordObj(self._id, id)),
        )
...
class BingoObject(object):
    ...
    def getCurrentSimilarityValue(self):
        self._indigo._setSessionId()
        return Bingo._checkResult(
            self._indigo, self._bingo._lib.bingoGetCurrentSimilarityValue(self._id)
        )

    def estimateRemainingResultsCount(self):
        self._indigo._setSessionId()
        return Bingo._checkResult(
            self._indigo, self._bingo._lib.bingoEstimateRemainingResultsCount(self._id)
        )

    def estimateRemainingResultsCountError(self):
        self._indigo._setSessionId()
        return Bingo._checkResult(
            self._indigo, self._bingo._lib.bingoEstimateRemainingResultsCountError(self._id)
        )
    ...
    def estimateRemainingTime(self):
        ...
        value = c_float()
        Bingo._checkResult(
            self._indigo, self._bingo._lib.bingoEstimateRemainingTime(self._id, pointer(value))
        )
        return value.value
